...
if [ -n "$PIDS" ]
then
  echo "Takehome is already running in process $PIDS";
  exit 1
fi
...
LOG_FILE=${LOGGING_PATH}/takehome.out
ERR_FILE=${LOGGING_PATH}/takehome.err
...
echo "Giving Takehome $waitTimeout seconds to start successfully"
...
do
  ...
  then
    echo "Takehome didn't start within $waitTimeout seconds."
    PIDS=$(pgrep java -lf | grep $APP_NAME | cut -d" " -f1);
    if [ -n "$PIDS" ]
	then
	  echo "Killing $PIDS";
...
if [ $retVal -eq 1 ]
then
  echo "Starting Takehome failed"
else
  echo "Starting Takehome succeeded"
fi
